...
contract V13OrderVerifierTest is Test {
    function testRealOrderProofVerifies() public {
        ...
        command[4] = "../../../relayer/scripts/generateV13Proof.ts"; command[5] = "order";
        (bytes memory proof, bytes32[] memory inputs) = abi.decode(vm.ffi(command), (bytes, bytes32[]));
        assertEq(inputs.length, 8); assertTrue(verifier.verify(proof, inputs));
    }
}
